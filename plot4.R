...
with(pow, plot(timeofday,gap,type = "n",ylab="Global Active Power",xlab=""))
with(pow, lines(timeofday,gap))

# plot voltage vs date time
with(pow, plot(timeofday,Voltage,type = "n",xlab="datetime"))
with(pow, lines(timeofday,Voltage))
...
with(pow, plot(timeofday,Global_reactive_power,type = "n",xlab = "datetime"))
with(pow, lines(timeofday,Global_reactive_power))
dev.off()